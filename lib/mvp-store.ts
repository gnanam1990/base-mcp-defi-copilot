...
const metricKinds = ["count", "paidRuns", "delta"] as MetricKind[];
...
function slugify(value: string) {
  const base = value
    ...
    .replace(/[^a-z0-9]+/g, "-")
    .replace(/^-+|-+$/g, "");
  return base || `defi-copilot-item-${Date.now()}`;
}
...
export function createItem(input: ItemInput) {
  ...
  const item: MvpItem = {
    ...
    payload: input.payload || {
      "recommendation": "move idle USDC to low-risk lending",
      "risk": "low"
    },
    createdAt: now,
    updatedAt: now,
...
export function recordPaidRun(
  ...
  const receipt: MvpReceipt = {
    id: `defi-copilot-${itemSlug}-${Date.now()}`,
    itemSlug,
    itemName: item.name,
