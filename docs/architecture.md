...
- File-backed portfolio state in `.data/defi-copilot-db.json`, seeded from the MVP records.
- Product status endpoint: `GET /api/defi-copilot/status`.
- Wallets endpoints for listing, creating, quoting, and paid execution.
...
- `app/api/defi-copilot/wallets` exposes creation and listing.
- `app/api/defi-copilot/wallets/[slug]/quote` returns a payment requirement.
- `app/api/defi-copilot/wallets/[slug]/run` blocks unpaid access with `402 Payment Required`, records paid runs, and emits `payment-response`.
- `app/api/mcp/defi-copilot` maps agent tools to discovery, quotes, prepared runs, and stats.
