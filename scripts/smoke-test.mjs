const baseUrl = process.env.DEFI_COPILOT_BASE_URL || "http://127.0.0.1:3007";
...
const status = await request("/api/defi-copilot/status");
...
const created = await request(
  "/api/defi-copilot/wallets",
  {
    method: "POST",
    headers: { "content-type": "application/json" },
    body: JSON.stringify({
      name: `Smoke portfolio ${Date.now()}`,
      descriptor: "+4.8% APY option",
      detail: "Low risk",
      priceUsdc: 0.1,
      payload: {"recommendation":"move idle USDC to low-risk lending","risk":"low"},
    }),
  },
  201,
);
...
assert(item.slug, "created portfolio should include a slug");

const listed = await request("/api/defi-copilot/wallets");
assert(
  listed.json.data.some((entry) => entry.slug === item.slug),
  "portfolio list should include created item",
);

const quote = await request(`/api/defi-copilot/wallets/${item.slug}/quote`);
...
const unpaid = await request(
  `/api/defi-copilot/wallets/${item.slug}/run`,
  {
    method: "POST",
    headers: { "content-type": "application/json" },
...
const paid = await request(`/api/defi-copilot/wallets/${item.slug}/run`, {
  method: "POST",
  headers: {
...
const mcp = await request("/api/mcp/defi-copilot", {
  method: "POST",
  headers: { "content-type": "application/json" },
  body: JSON.stringify({
    tool: "get_portfolio_quote",
